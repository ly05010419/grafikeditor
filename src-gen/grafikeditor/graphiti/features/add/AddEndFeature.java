...
public class AddEndFeature extends CincoAbstractAddFeature {
	...
	public PictogramElement add(IAddContext context) {
		grafikeditor.meineditor.End end = (grafikeditor.meineditor.End) context.getNewObject();
		if (end.getId() == null || end.getId().isEmpty())
			end.setId(EcoreUtil.generateUUID());
		...
		//MeinEditorGraphitiUtils.getInstance().addImage("icons/Stop.png", "icons/Stop.png");
		ContainerShape Shape0 = peService.createContainerShape(d, true);
		Image image0 = gaService.createImage(Shape0, "icons/Stop.png");
		...
		MeinEditorEContentAdapter.getInstance().addAdapter(end);

		linkAllShapes(Shape0, end);
		//link(Shape0, end);
