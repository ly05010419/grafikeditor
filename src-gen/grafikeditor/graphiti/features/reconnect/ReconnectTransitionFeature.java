...
public class ReconnectTransitionFeature extends DefaultReconnectionFeature{
	...
	public ReconnectTransitionFeature(IFeatureProvider fp) {
		super(fp);
	}
		...
		if (apiCall) {
			...
			if (newAnchor != null) {
				...
				if (ReconnectionContext.RECONNECT_TARGET.equals(context.getReconnectType())) {
					return ((graphmodel.Node) bo).canEnd(grafikeditor.meineditor.Transition.class);
				}
				
				if (ReconnectionContext.RECONNECT_SOURCE.equals(context.getReconnectType())) {
					return ((graphmodel.Node) bo).canStart(grafikeditor.meineditor.Transition.class);
				}
			}
			return false;
		}
