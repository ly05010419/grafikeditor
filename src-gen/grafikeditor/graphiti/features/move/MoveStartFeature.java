...
public class MoveStartFeature extends CincoMoveShapeFeature{

	private ECincoError error = ECincoError.OK;

	public MoveStartFeature(IFeatureProvider fp) {
		super(fp);
	...
	public boolean canMoveShape(IMoveShapeContext context, boolean apiCall) {
		if (apiCall) {
			...
			if (target instanceof graphmodel.ModelElementContainer)
				return ((graphmodel.ModelElementContainer) target).canContain(grafikeditor.meineditor.Start.class);
	...
	public void moveShape(IMoveShapeContext context) {
		...
		if (source instanceof Container) {
			...
			nc.getModelElements().remove((grafikeditor.meineditor.Start) o);
		}
		if (source instanceof grafikeditor.meineditor.MeinEditor) {
			...
			tmp.getModelElements().remove((grafikeditor.meineditor.Start) o);
		}
		if (target instanceof grafikeditor.meineditor.MeinEditor) {
			...
			tmp.getModelElements().add((grafikeditor.meineditor.Start) o);
		}
		...
		grafikeditor.meineditor.Start tmp = (grafikeditor.meineditor.Start) o;
	...
	protected void postMoveShape(IMoveShapeContext context) {
		try {
			grafikeditor.meineditor.Start _s = (grafikeditor.meineditor.Start) getBusinessObjectForPictogramElement(context.getPictogramElement());
			...
			grafikeditor.api.cmeineditor.CStart cModelElement= _wrapped.findCStart(_s);
