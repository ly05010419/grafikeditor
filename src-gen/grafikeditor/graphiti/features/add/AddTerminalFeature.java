...
public class AddTerminalFeature extends CincoAbstractAddFeature {
	...
	public PictogramElement add(IAddContext context) {
		grafikeditor.meineditor.Terminal terminal = (grafikeditor.meineditor.Terminal) context.getNewObject();
		if (terminal.getId() == null || terminal.getId().isEmpty())
			terminal.setId(EcoreUtil.generateUUID());
		...
		Ellipse ellipse0 = gaService.createPlainEllipse(Shape0);
		...
		if (context.getWidth() < 0 && context.getHeight() < 0) {
			...
			gaService.setLocation(ellipse0, context.getX(), context.getY());
		}
		peService.setPropertyValue(ellipse0, MeinEditorLayoutUtils.KEY_HORIZONTAL, MeinEditorLayoutUtils.KEY_HORIZONTAL_UNDEFINED);
		peService.setPropertyValue(ellipse0, MeinEditorLayoutUtils.KEY_VERTICAL, MeinEditorLayoutUtils.KEY_VERTICAL_UNDEFINED);
		...
		try {
			...
			Thread.currentThread().setContextClassLoader(AddTerminalFeature.class.getClassLoader());
		...
		gaService.setLocation(text0, -7, -7);
		...
		MeinEditorEContentAdapter.getInstance().addAdapter(terminal);

		linkAllShapes(Shape0, terminal);
		//link(Shape0, terminal);
