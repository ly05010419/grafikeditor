...
public class MoveGatewayFeature extends CincoMoveShapeFeature{
	...
	public boolean canMoveShape(IMoveShapeContext context, boolean apiCall) {
		if (apiCall) {
			...
			if (target instanceof graphmodel.ModelElementContainer)
				return ((graphmodel.ModelElementContainer) target).canContain(grafikeditor.meineditor.Gateway.class);
	...
	public void moveShape(IMoveShapeContext context) {
		...
		if (source instanceof Container) {
			...
			nc.getModelElements().remove((grafikeditor.meineditor.Gateway) o);
		}
		if (source instanceof grafikeditor.meineditor.MeinEditor) {
			...
			tmp.getModelElements().remove((grafikeditor.meineditor.Gateway) o);
		}
		if (target instanceof grafikeditor.meineditor.MeinEditor) {
			...
			tmp.getModelElements().add((grafikeditor.meineditor.Gateway) o);
		}
		...
		grafikeditor.meineditor.Gateway tmp = (grafikeditor.meineditor.Gateway) o;
	...
	protected void postMoveShape(IMoveShapeContext context) {
		try {
			grafikeditor.meineditor.Gateway _s = (grafikeditor.meineditor.Gateway) getBusinessObjectForPictogramElement(context.getPictogramElement());
			...
			grafikeditor.api.cmeineditor.CGateway cModelElement= _wrapped.findCGateway(_s);
