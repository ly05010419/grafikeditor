...
public class MoveVariableFeature extends CincoMoveShapeFeature{

	private ECincoError error = ECincoError.OK;

	public MoveVariableFeature(IFeatureProvider fp) {
		super(fp);
	...
	public boolean canMoveShape(IMoveShapeContext context, boolean apiCall) {
		if (apiCall) {
			...
			if (target instanceof graphmodel.ModelElementContainer)
				return ((graphmodel.ModelElementContainer) target).canContain(grafikeditor.meineditor.Variable.class);
	...
	public void moveShape(IMoveShapeContext context) {
		...
		if (source instanceof Container) {
			...
			nc.getModelElements().remove((grafikeditor.meineditor.Variable) o);
		}
		if (source instanceof grafikeditor.meineditor.MeinEditor) {
			...
			tmp.getModelElements().remove((grafikeditor.meineditor.Variable) o);
		}
		if (target instanceof grafikeditor.meineditor.MeinEditor) {
			...
			tmp.getModelElements().add((grafikeditor.meineditor.Variable) o);
		}
		...
		grafikeditor.meineditor.Variable tmp = (grafikeditor.meineditor.Variable) o;
	...
	protected void postMoveShape(IMoveShapeContext context) {
		try {
			grafikeditor.meineditor.Variable _s = (grafikeditor.meineditor.Variable) getBusinessObjectForPictogramElement(context.getPictogramElement());
			...
			grafikeditor.api.cmeineditor.CVariable cModelElement= _wrapped.findCVariable(_s);
